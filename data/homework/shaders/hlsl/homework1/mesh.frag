...
float3 calculateNormal(VSOutput input)
{
    ...
    float3 tangentNormal = textureColorMap[materialCBO.normalTextureIndex].Sample(samplerColorMap[materialCBO.normalTextureIndex], input.UV).xyz * 2.0 - 1.0;
...
float4 main(VSOutput input) : SV_TARGET
{
    ...
    if(materialCBO.occlusionTextureIndex != -1)
    {
        return textureColorMap[materialCBO.occlusionTextureIndex].Sample(samplerColorMap[materialCBO.occlusionTextureIndex], input.UV);
        
    }
    else
    {
        return textureColorMap[materialCBO.normalTextureIndex].Sample(samplerColorMap[materialCBO.normalTextureIndex], input.UV);
